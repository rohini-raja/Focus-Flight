...
type RailPhase = 'picker' | 'active' | 'arrived';
...
export function RailFocusView({ onEnd }: RailFocusViewProps) {
  ...
  const totalDistKm =
    depStation && arrStation ? haversineKm(depStation, arrStation) : 0;
  ...
  useEffect(() => {
    if (phase !== 'active' || !mapContainerRef.current || !depStation || !arrStation) return;
  ...
  const handleSearch = async (which: 'dep' | 'arr') => {
    ...
    if (which === 'dep') {
      ...
      setArrResults(results);
      setIsSearchingArr(false);
    }
  };
        ...
        <main className="max-w-2xl mx-auto space-y-8">
          <div>
            ...
            <p className="text-muted-foreground">
              Live train maps powered by OpenRailwayMap. Your session rides real tracks.
            </p>
          </div>
                ...
                >
                  <div className="text-sm">
                    ...
                    <div className="text-muted-foreground mt-0.5">
                      {Math.round(totalDistKm).toLocaleString()} km route · {duration}m session
                    </div>
                  </div>
          ...
          {/* Popular stations quick-fill */}
          <div className="space-y-2">
            <div className="text-xs text-muted-foreground uppercase tracking-wider">
              Quick-fill departure
            </div>
            <div className="flex flex-wrap gap-2">
          ...
          <button
            onClick={() => depStation && arrStation && setPhase('active')}
            disabled={!depStation || !arrStation}
      ...
      >
        <div className="text-center max-w-md space-y-6">
          ...
          <p className="text-xl font-semibold" style={{ color: GREEN }}>
            Welcome to {arrStation?.name}
          </p>
          <div className="bg-card/50 border border-white/10 rounded-2xl p-6 space-y-3 text-left">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="text-white font-medium">{duration}m · {focusType}</span>
            </div>
          </div>
          <button
            onClick={onEnd}
            className="w-full py-3 rounded-xl font-bold text-background hover:opacity-90 transition-opacity"
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 8000, background: '#0a0e1a' }}>
      ...
      {/* Fixed train icon at screen center */}
      <div
        style={{
          position: 'fixed',
          top: '50%',
      ...
      {/* Top-left HUD: pause + route info + end early */}
      <div
        style={{
          position: 'fixed',
          top: 20,
        ...
        <div style={{ ...GLASS, padding: '12px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
          <button
            ...
            style={{
              ...
              height: 40,
              borderRadius: 11,
              background: isActive ? 'rgba(255,255,255,0.12)' : GREEN,
              border: 'none',
              cursor: 'pointer',
              display: 'flex',
              alignItems: 'center',
          ...
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: 'white', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 240 }}>
              {depStation?.name} → {arrStation?.name}
            </div>
      ...
      {/* Bottom HUD: Time Remaining (left) + Distance Remaining (right) */}
      <div
        style={{
          position: 'fixed',
          bottom: 36,
      ...
      >
        ...
        <div style={{ ...GLASS, padding: '16px 24px', textAlign: 'right', pointerEvents: 'auto' }}>
          ...
          >
            {distKmLeft < 10
              ? distKmLeft.toFixed(1)
              : Math.round(distKmLeft).toLocaleString()}
            <span
              style={{
                fontSize: 16,
                fontWeight: 700,
        ...
        {showExit && (
          ...
          >
            ...
            >
              ...
              >
                This session won't be logged as complete.
              </p>
